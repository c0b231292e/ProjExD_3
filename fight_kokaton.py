...
class Score:
    def __init__(self,height) -> None:
        ...
        self.rect = self.img.get_rect(center=(100, HEIGHT - 50))  # 左下に位置する座標
    
    def update(self, screen: pg.Surface,sco:int):
        self.score = sco
        self.img = self.fonto.render(f"スコア: {self.score}", 0, (0,0,255))
        self.rect = self.img.get_rect(center=(100, HEIGHT - 50))  # 左下に表示する位置
        # 画面にスコアを描画
        ...
        for j,bomb in enumerate (bombs):
            ...
            if beam is not None and bomb is not None:      
                if beam.rct.colliderect(bomb.rct):  # ビームと爆弾が衝突したら
                    ...
                    #ls[i], bombs[j] = None, None
                    bird.change_img(6, screen)
                    scores += 1
                    pg.display.update()

        bombs = [bomb for bomb in bombs if bomb is not None]
        # ls = [beam for beam in ls if beam is not None]
        # ls = [beam for beam in ls if check_bound(beam.rct) == (True,True)]
        key_lst = pg.key.get_pressed()
        bird.update(key_lst, screen)
        score.update(screen,scores)
        if beam is not None:
            beam.update(screen) 
        for bomb in bombs:
            bomb.update(screen)
